...
class TestCoreCLIIntegration:
    ...
    def test_engine_process_diff_integration(self):
        ...
        try:
            # Create patches
            with patch("committy.core.engine.load_config") as mock_load_config, \
                 patch("committy.core.engine.get_diff") as mock_get_diff, \
                 patch("committy.core.engine.generate_commit_message") as mock_generate:
                
                # Setup mocks
                mock_load_config.return_value = {"model": "test-model", "format": "conventional"}
                mock_get_diff.return_value = "test diff"
                mock_generate.return_value = "feat(test): add new feature"
